...
class USD2026Cleaner(BaseCleaner):
    """Limpiador específico para USD 2026"""
    
    def clean(self):
        # Validar hojas requeridas
        required_sheets = ["USD", "Maestro recaladas", "Servicios USD", 
                           "Clase carga USD", "Clientes USD", "Tipo Ingreso"]
        self._validate_required_sheets(required_sheets)
        
        # Leer las hojas
        df_usd = pd.read_excel(self.xls, "USD")
        df_maestro = pd.read_excel(self.xls, "Maestro recaladas")
        df_servicios = pd.read_excel(self.xls, "Servicios USD")
        df_clase_carga = pd.read_excel(self.xls, "Clase carga USD")
        df_clientes = pd.read_excel(self.xls, "Clientes USD")
        df_tipo_ingreso = pd.read_excel(self.xls, "Tipo Ingreso")
        
        self.stats['registros_iniciales'] = len(df_usd)
        self.stats['columnas_iniciales'] = len(df_usd.columns)
        ...
        self.stats['modificaciones'].append('Régimen completado y corregido')
        ...
        self.stats['modificaciones'].append('Servicios 2 insertado')
        ...
        self.stats['modificaciones'].append('Tipo insertado')
        ...
        self.stats['modificaciones'].append('Clase Carga completada')
        ...
        self.stats['modificaciones'].append('Reglas especiales aplicadas a Clase Carga')
        ...
        self.stats['modificaciones'].append('Tipo Carga asignado')
        ...
        self.stats['registros_finales'] = len(df_usd)
        self.stats['columnas_finales'] = len(df_usd.columns)
        
        return df_usd, self.stats